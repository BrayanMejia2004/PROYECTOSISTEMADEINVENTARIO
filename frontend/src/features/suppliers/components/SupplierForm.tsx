...
import { supplierSchema, type SupplierForm } from '../schemas';
...
export const SupplierFormComponent = ({ defaultValues, isPending, onSubmit, onCancel }: Props) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-6 space-y-4">
      <h3 className="font-sans font-semibold text-brand-text mb-4">
        {defaultValues ? 'Editar Proveedor' : 'Nuevo Proveedor'}
      </h3>
