...
### 2026-05-22 — Departments as per-branch entities
**Contexto:** Department schema had no `branchId` field, but the service and controller always created and queried departments with `branchId`. Mongoose's `strict: true` silently stripped the field on save, making all department documents lack `branchId` in the DB. Queries with a `branchId` filter returned empty for all non-owner users.
**Decisión:** Added `branchId` (optional) to Department schema. Updated the unique index to `{ tenantId, branchId, name }`. Modified all service queries to include departments without `branchId` (backward compatibility for existing data).
**Alternativas descartadas:** Stripping branchId filtering entirely (departments are explicitly per-branch in the UI — admin/cashier users create them for their branch). Migrating existing data with a script (no automated migrations set up).
**Consecuencias:** New departments will be saved with `branchId`. Existing departments without `branchId` will still appear thanks to the `$or` fallback. When re-created by users, they'll get properly associated with a branch.
