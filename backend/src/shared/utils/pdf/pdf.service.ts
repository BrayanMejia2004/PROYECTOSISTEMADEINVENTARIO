...
function center(doc: PDFKit.PDFDocument, text: string, size: number, bold: boolean = false): void {
  doc.fontSize(size).font(bold ? 'Helvetica-Bold' : 'Helvetica').text(text, { align: 'center' });
}

function drawSeparator(doc: PDFKit.PDFDocument, left: number, pageWidth: number, margin: number = 0): void {
  doc.moveDown(margin);
  doc.moveTo(left, doc.y).lineTo(left + pageWidth, doc.y).stroke('#cccccc');
  doc.moveDown(margin);
}

function drawHeader(sale: SaleData, doc: PDFKit.PDFDocument, left: number, pageWidth: number): void {
  center(doc, sale.tenantName, 18, true);
  doc.moveDown(0.2);
  if (sale.tenantNit) center(doc, `NIT: ${sale.tenantNit}`, 9);
  if (sale.tenantAddress) center(doc, sale.tenantAddress, 9);
  if (sale.tenantPhone) center(doc, `Tel: ${sale.tenantPhone}`, 9);
  if (sale.tenantEmail) center(doc, sale.tenantEmail, 9);
  center(doc, `Sucursal: ${sale.branchName}`, 9);

  drawSeparator(doc, left, pageWidth, 0.8);

  center(doc, 'Comprobante de Venta', 14, true);
  doc.moveDown(0.3);
  center(doc, `N°: ${sale.saleNumber}`, 10);
  center(doc, `Fecha: ${sale.createdAt.toLocaleDateString(LOCALE, { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}`, 9);
}

function drawCustomerInfo(doc: PDFKit.PDFDocument, sale: SaleData, left: number, pageWidth: number): void {
  drawSeparator(doc, left, pageWidth, 0.8);
...
function drawPaymentInfo(doc: PDFKit.PDFDocument, sale: SaleData, left: number, pageWidth: number): void {
  drawSeparator(doc, left, pageWidth, 0.5);
...
function drawItemsTable(doc: PDFKit.PDFDocument, sale: SaleData, left: number, pageWidth: number): void {
  drawSeparator(doc, left, pageWidth, 0.5);
  ...
  drawSeparator(doc, left, pageWidth, 0.3);
...
function drawTotals(doc: PDFKit.PDFDocument, sale: SaleData, left: number, pageWidth: number): void {
  ...
  function line(label: string, value: string, bold: boolean = false): void {
...
export const generateSalePdf = (sale: SaleData): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    ...
    drawHeader(sale, doc, left, pageWidth);
    drawCustomerInfo(doc, sale, left, pageWidth);
    drawPaymentInfo(doc, sale, left, pageWidth);
    drawItemsTable(doc, sale, left, pageWidth);
    drawTotals(doc, sale, left, pageWidth);
